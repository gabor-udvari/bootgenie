<nav class="tab_menu header_menu" id="header_userinfo">
    ...
    -->
    <ul class="nav navbar-right yamm">
        <?php if (false && !$tbg_user->isGuest()): ?>
        ...
        <li<?php if ($tbg_request->hasCookie('tbg3_original_username')): ?> class="temporarily_switched"<?php endif; ?> id="header_usermenu_link" class="dropdown">
            <?php if ($tbg_user->isGuest()): ?>
                <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false"><?php echo image_tag($tbg_user->getAvatarURL(true), array('alt' => '[avatar]', 'class' => 'guest_avatar'), true) . __('You are not logged in'); ?></a>
            <?php else: ?>
